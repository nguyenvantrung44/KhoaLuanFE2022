...
export default function ThongTinLopHocPhan(props) {
  ...
  const getThongTinLopHoc = () => {
    return data.filter((lop) => {
      return lop.id == id;
    })[0];
  }; 
  useEffect(() => {
    dispatch(atcGetSinhViensLopHocPhan(id));
    dispatch(atcGetThongBaoSinhViensLopHocPhan(id));
    dispatch(atcGetDonXinNghiHoc(localStorage.getItem("id"),id));
