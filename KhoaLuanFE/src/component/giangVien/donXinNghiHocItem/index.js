...
export default function DonXinNghiHocItem(props) {
  ...
  const onClickDonNghiHoc = () => {
    confirmAlert({
      ...
      message: `Sinh viên có mã số sinh viên: ${item.maSinhVien}, Họ tên:${
        item.hoTen
  ...
  return (
    <tr>
      <td>{props.stt}</td>
      <td onClick={onClickDonNghiHoc}> <input type="button" className="btn btn-link" value={item.maSinhVien}/></td>
      <td>{item.hoTen}</td>
